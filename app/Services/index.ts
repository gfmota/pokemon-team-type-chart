...
export const getPokemonList = async () => {
  const response = await fetch(
    `${POKEAPI_ROUTE}/pokemon?limit=1154`
  );
...
export const getTypeRelations = async (type: string) => {
  ...
  const { damage_relations: { double_damage_from: doubleDamageFrom, half_damage_from: halfDamageFrom, no_damage_from: noDamageFrom} } = await response.json();
